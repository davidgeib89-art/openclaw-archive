...
2.  **Diff:** We review exactly _what_ changed in the code/logic.
3.  **Reflect:** We discuss _why_ it changed. Does it align with the Soul? Is it safe?
...
We are currently waiting for Codex to write the _Ledger Entry_ for R081.
...
- **Action:** We will challenge Codex if he proposes excessive refactoring for minor gains. We prioritize _Momentum_ over _Micro-Optimizations_.
...
- **Reflection:** This is a _format_ error, not a _safety_ error. The ritual content is likely safe, but the structure is wrong.
...
- **Directive:**
  ...
  4.  **IMMEDIATELY** switch to Phase B (Memory Consolidation) and begin the Graph Memory implementation.
      _Momentum Rule applies: One confirmation is safety. Two is stalling._
...
- **Action:**
  ...
  2.  **Use Spot Checks:** For minor fixes (like R01), run _only_ that specific ritual (`R01-only`).
...
- **Verification:** Checked `episodic-memory.ts` and `decision.ts`.
  - **Finding:** Graph Memory Scaffold (SQLite table, Triple Extraction, Recall Injection) is _already implemented_ and looks correct.
...
- **Next Step:** Since V1 works, we do NOT need V2 (Soft Decay). We proceed to **"Active Usage"**.
  - _Directive:_ Enable the feature globally (if behind feature flag) and monitor `graphConflictsResolved` metric.
...
- **Decision:**
  ...
  - _Rationale:_ Efficiency (Tokens/Latency). Keep it small. Don't reopen the whole brain.
...
- **Code:** `om-scaffolding.ts` now scans only the _latest_ stamped user segment for refusals. Injected context is ignored.
...
- **Next:** **Phase C - Active Usage & Learning Governance**.
  - _Directive:_ "Use the Brain." We observe behavior in the wild before adding "Self-Update" logic.
