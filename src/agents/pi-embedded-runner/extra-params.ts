  ...
  const wrappedStreamFn: StreamFn = (model, context, options) => {
    ...
    return Promise.resolve(streamPromise).then((stream) => {
      ...
      const isMiniMax =
        ...
        modelIdLower.includes("trinity") ||
        modelIdLower.includes("arcee");
      ...
      if (anyStream[Symbol.asyncIterator]) {
        ...
        anyStream[Symbol.asyncIterator] = async function* () {
          ...
          debugLog(
            `[Stream-Debug] Interception monitor active for ${model.id} (isMiniMax=${isMiniMax})`,
          );
          ...
          try {
            for await (const chunk of originalIterator()) {
              ...
              if (currentText) {
                ...
                if (buffer.length > 0 && currentText.length > 50 && currentText.startsWith(buffer))
                  buffer = currentText;
                ...
                while (changed) {
                  ...
                  if (!inToolBlock) {
                    ...
                    if (match && match.index !== undefined) {
                      ...
                      if (sIdx > 0)
                        yield { type: "text_delta", delta: buffer.substring(0, sIdx) } as any;
                  ...
                  if (inToolBlock) {
                    ...
                    if (match && match.index !== undefined) {
                      ...
                      debugLog(
                        `[Stream-Debug] FOUND END MARKER. Content length: ${toolContent.length}`,
                      );

                      try {
                        const cleanContent = toolContent
                          .replace(/```json/g, "")
                          .replace(/```/g, "")
                          .trim();
                        const rawCalls = cleanContent
                          .split(/<[^>]*?tool[^>]*?call[^>]*?begin[^>]*?>/i)
                          .filter((s) => s.trim());
                        ...
                        debugLog("[Stream-Debug] Parser Error", e);
                      }
                      inToolBlock = false;
                      changed = true;
                    } else {
